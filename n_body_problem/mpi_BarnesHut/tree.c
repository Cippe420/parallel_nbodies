...
void Tree__merge(struct node *root, struct node *recv_node){
    ...
    // se il nodo sorgente è vuoto, non c'è niente da fare
    if (recv_node == NULL){
        return;
    }

    // unisci i valori di massa e centro di massa
    double total_mass = root->mass + recv_node->mass;
    root->com[0] = (root->com[0] * root->mass + recv_node->com[0] * recv_node->mass) / total_mass;
    root->com[1] = (root->com[1] * root->mass + recv_node->com[1] * recv_node->mass) / total_mass;

    // ricorsivamente unisci i figli
    if (recv_node->ne != NULL){

        // se il figlio target è null, copialo dal source
        if(!root->ne){
            root->ne = recv_node->ne;
            recv_node->ne = NULL;
        }else{
            // altrimenti,unisci i figli ricorsivamente
            Tree__merge(root->ne,recv_node->ne);
        }
    }

    if (recv_node->se != NULL){

        if(!root->se){
            root->se = recv_node->se;
            recv_node->se = NULL;
        }else{
            Tree__merge(root->se,recv_node->se);
        }
    }

    if (recv_node->nw !=NULL){

        if(!root->nw){
            root->nw = recv_node->nw;
            recv_node->nw = NULL;
        }else{
            Tree__merge(root->nw,recv_node->nw);
        }
    }

    if (recv_node->sw != NULL){

        if(!root->sw){
            root->sw = recv_node->sw;
            recv_node->sw = NULL;
        }else{
            Tree__merge(root->sw,recv_node->sw);
        }
    }
    // libera il nodo in ricevimento dopo il merge
